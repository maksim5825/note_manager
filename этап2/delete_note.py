...
def delete_note_username(zaproc2):
    global list_note
    for i in range(len(list_note)):
        if 'имя пользователя:' in list_note[i] or 'заголовок заметки:' in list_note[i]:
            del list_note[i]
            print("заметка успешно удалена. Оставшиеся заметки:")
        else:
...
def program():
    ...
    while 1 > 0:
        ...
        if zaproc != "да":
            break
    print("Текущие заметки:", sep="\n" )
    print_note()
    zaproc2 = input("Введите имя пользователя или заголовок для удаления заметки:")
    delete_note_username(zaproc2)
    print_note()
program()
